...
在 `docs/specs/_architecture/` 下生成 7 个文件（5 个后端 + 2 个前端），使用 `templates/architecture/` 模板：
...
| `FRONTEND_STYLE.md` | `docs/specs/_architecture/FRONTEND_STYLE.md` | 根据 `frontend` 段和 constitution 提取技术栈、目录结构、命名规范、API 调用层、状态管理、路由、样式、测试规范 |
| `COMPONENT_LIBRARY.md` | `docs/specs/_architecture/COMPONENT_LIBRARY.md` | 根据 features 提取通用组件、布局组件、业务组件需求，登记组件注册表 |
...
- 前端模板（FRONTEND_STYLE.md、COMPONENT_LIBRARY.md）仅在 `project.yaml` 包含 `frontend` 段或项目类型为全栈时生成
...
- FRONTEND_STYLE.md（前端规范）
- COMPONENT_LIBRARY.md（组件注册表）
