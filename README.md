...
> Schema 冻结状态、Phase 记录、变更政策等治理信息见 [`docs/governance/GOVERNANCE.md`](docs/governance/GOVERNANCE.md)。
...
├── USAGE.md                     # 详细使用说明
├── docs/
│   └── governance/              # 框架治理文档（Schema 冻结、审查记录）
...
└── templates/                   # 模板文件（只含会被复制到用户项目的内容）
    ├── architecture/            # 架构规格模板（7 个：5 后端 + 2 前端）
    ├── changes/                 # 变更文档模板（4 个）
    ├── meta/                    # 元文档模板（11 个）+ Schema 定义（6 个）
    ├── orders/                  # 工单模板
    ├── reference/               # 功能建议目录
    └── specs/                   # 域规格模板
```
